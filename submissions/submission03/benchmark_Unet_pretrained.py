...
def Unet(pretrained_weights = None):
    ...
    cnn.compile(optimizer = Adam(lr = 1e-7), loss = 'binary_crossentropy', metrics = ['accuracy'])

    #model.summary()

    if(pretrained_weights):
    	cnn.load_weights(r'.\Unet_freezed2000.h5')
...
#iamges, masks, segmentations = md.mutate_data(images, masks, segmentations)
...
cnn = Unet(pretrained_weights = True)
#and start training
if trainnetwork:
    losslist = []
